...
private String buildLoginTicketXml() {
    ...
    java.time.ZonedDateTime nowArgentina = java.time.ZonedDateTime.now(zoneArgentina);
    ...
    return "<?xml version=\"1.0\" encoding=\"UTF-8\" standalone=\"yes\"?>\n" +
            ...
            "    <destination>" + dstDN + "</destination>\n" +
            "    <uniqueId>" + (System.currentTimeMillis() / 1000) + "</uniqueId>\n" +
            "    <generationTime>" + nowArgentina.minusMinutes(5).format(formatter) + "-03:00</generationTime>\n" +
            "    <expirationTime>" + nowArgentina.plusHours(2).format(formatter) + "-03:00</expirationTime>\n" +
            "  </header>\n" +
            "  <service>ws_sr_padron_a13</service>\n" +
            "</loginTicketRequest>";
}
